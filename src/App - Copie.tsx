...
import { 
  ...
  Settings, LogOut, Menu, X, ScrollText, DollarSign
} from 'lucide-react';
...
const MainLayout: React.FC<{user: User, onLogout: () => void}> = ({ user, onLogout }) => {
  ...
  const navItems = [
    ...
    { label: 'Ordonnances', icon: ScrollText, path: '/prescriptions' },
    { label: 'Facturation & CNSS', icon: FileText, path: '/billing' },
    { label: 'Dépenses & Recettes', icon: DollarSign, path: '/finance' },
  ];
      ...
      >
        <div className="h-full flex flex-col">
          <div className="h-16 flex items-center px-6 border-b border-slate-100">
            <div className="w-8 h-8 bg-medical-600 rounded-lg flex items-center justify-center text-white font-bold text-lg mr-3">
              C
            </div>
          ...
          <div className="p-4 border-t border-slate-100">
            <button 
                onClick={onLogout}
                className="flex items-center gap-3 px-4 py-3 w-full text-slate-600 hover:bg-red-50 hover:text-red-600 rounded-lg transition"
            >
              <LogOut size={20} />
