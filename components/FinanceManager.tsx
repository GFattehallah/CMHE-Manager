...
export const FinanceManager: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    const periodLabel = isAnnualView ? `${filterYear}` : `${filterMonth + 1}_${filterYear}`;
    const fileName = activeTab === 'overview' ? `Finance_Cabinet_${periodLabel}.xlsx` : 
                    activeTab === 'revenues' ? `Recettes_${periodLabel}.xlsx` : `Depenses_${periodLabel}.xlsx`;
    ...
    <div className="p-6 h-[calc(100vh-80px)] flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-500 font-medium ml-1">Gestion des flux monétaires du cabinet</p>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={exportToExcel}
            className="bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-2xl flex items-center gap-2 hover:bg-slate-50 hover:border-emerald-500 hover:text-emerald-600 transition-all active:scale-95 font-bold shadow-sm"
          >
            <FileSpreadsheet size={18} /> 
            <span className="hidden sm:inline">Exporter Excel</span>
            <span className="sm:hidden text-xs">Excel</span>
          </button>
          <button onClick={() => setIsRevenueModalOpen(true)} className="bg-emerald-600 text-white px-5 py-2.5 rounded-2xl flex items-center gap-2 shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95 font-bold">
            <TrendingUp size={20}/> <span className="hidden sm:inline">Recette</span>
          </button>
          <button onClick={() => setIsExpenseModalOpen(true)} className="bg-rose-600 text-white px-5 py-2.5 rounded-2xl flex items-center gap-2 shadow-lg shadow-rose-100 hover:bg-rose-700 transition-all active:scale-95 font-bold">
            <TrendingDown size={20}/> <span className="hidden sm:inline">Dépense</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-6 items-center justify-between bg-white p-2 rounded-[2rem] border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-4 w-full lg:w-auto px-2">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button onClick={() => setIsAnnualView(false)} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition ${!isAnnualView ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>Mois</button>
            <button onClick={() => setIsAnnualView(true)} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition ${isAnnualView ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>Année</button>
          </div>
          <div className="h-8 w-px bg-slate-200 hidden lg:block"></div>
          <div className="flex items-center gap-2">
            {!isAnnualView && (
              <select value={filterMonth} onChange={(e) => setFilterMonth(parseInt(e.target.value))} className="bg-transparent text-sm font-black text-slate-700 outline-none">
                {Array.from({ length: 12 }).map((_, i) => (
                  <option key={i} value={i}>{new Date(0, i).toLocaleString('fr-FR', { month: 'long' })}</option>
                ))}
              </select>
            )}
            <select value={filterYear} onChange={(e) => setFilterYear(parseInt(e.target.value))} className="bg-transparent text-sm font-black text-slate-700 outline-none border-l pl-2 border-slate-200">
          ...
          <div className="space-y-6 animate-fade-in pb-10">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
                ...
                <div className="p-4 bg-emerald-50 rounded-2xl text-emerald-600"><TrendingUp size={28}/></div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                ...
                <div className="p-4 bg-rose-50 rounded-2xl text-rose-600"><TrendingDown size={28}/></div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-md flex items-center justify-between bg-gradient-to-br from-white to-slate-50">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Bénéfice Net Période</p>
                ...
                <div className="p-4 bg-indigo-50 rounded-2xl text-indigo-600"><Coins size={28}/></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm h-[450px]">
                <h3 className="font-black text-slate-800 flex items-center gap-2 uppercase tracking-tighter text-lg mb-8">
                  <BarChart3 size={20} className="text-indigo-600"/> Comparatif Flux Financiers
                </h3>
                <div className="h-80">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm h-[450px]">
                <h3 className="font-black text-slate-800 flex items-center gap-2 uppercase tracking-tighter text-lg mb-8">
                  <PieIcon size={20} className="text-orange-500"/> Répartition Dépenses
                </h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={expenseByCategory} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={8} dataKey="value">
                        {expenseByCategory.map((entry, index) => <Cell key={`cell-${index}`} fill={(CATEGORY_COLORS as any)[entry.key] || '#cbd5e1'} />)}
                      </Pie>
                      <RechartsTooltip />
                    </PieChart>
                  </ResponsiveContainer>
                ...
                {currentPeriodData.invoices.map(inv => (
                  <tr key={inv.id} className="hover:bg-emerald-50/30 transition-colors group">
                    ...
                    <td className="p-5 text-center pr-8">
                      <button onClick={() => handleDeleteRevenue(inv.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
                {currentPeriodData.invoices.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-20 text-center text-slate-300 font-bold uppercase tracking-widest">Aucune recette pour cette période</td>
                  </tr>
                )}
              </tbody>
            </table>
              ...
              <tbody className="divide-y divide-slate-50">
                {currentPeriodData.expenses.map(exp => (
                  <tr key={exp.id} className="hover:bg-rose-50/30 transition-colors group">
                    ...
                    <td className="p-5">
                      <div className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-slate-100 inline-block mb-1" style={{ color: (CATEGORY_COLORS as any)[exp.category] }}>{EXPENSE_CATEGORIES[exp.category]}</div>
                      <div className="font-bold text-slate-800 text-sm">{exp.description}</div>
                    </td>
                    <td className="p-5 text-[10px] font-black uppercase text-slate-500">{exp.paymentMethod}</td>
                    <td className="p-5 text-right font-black text-rose-600 text-sm">-{formatMAD(exp.amount)} MAD</td>
                    <td className="p-5 text-center pr-8">
                      <button onClick={() => handleDeleteExpense(exp.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
                {currentPeriodData.expenses.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-20 text-center text-slate-300 font-bold uppercase tracking-widest">Aucune dépense pour cette période</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {isExpenseModalOpen && (
        <ExpenseModal onClose={() => setIsExpenseModalOpen(false)} onSave={(exp: Expense) => { DataService.saveExpense(exp); loadData(); setIsExpenseModalOpen(false); }} />
      )}
      {isRevenueModalOpen && (
        <RevenueModal patients={patients} onClose={() => setIsRevenueModalOpen(false)} onSave={(inv: Invoice) => { DataService.saveInvoice(inv); loadData(); setIsRevenueModalOpen(false); }} />
      )}
    </div>
  );
};
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input name="date" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none font-bold"/>
                        </div>
                        <div>
                    ...
                    <div>
                      ...
                      <input name="description" required placeholder="Ex: Loyer, Electricité..." className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none font-medium"/>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Catégorie</label>
                          <select name="category" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none font-bold">
                        ...
                        <div>
                          ...
                          <select name="paymentMethod" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none font-bold">
...
const RevenueModal = ({ patients, onClose, onSave }: any) => {
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        onSave({
            ...
            status: 'PAID',
            paymentMethod: formData.get('paymentMethod') as PaymentType,
            items: [{ description: formData.get('description') as string || 'Recette', price: Number(formData.get('amount')) }]
        });
    };
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-fade-in border border-slate-200">
                <div className="p-8 bg-emerald-600 text-white flex justify-between items-center">
                  <h2 className="font-black uppercase tracking-widest text-sm flex items-center gap-2"><TrendingUp size={18}/> Nouvelle Recette Cash</h2>
                  <button onClick={onClose} className="text-3xl font-light hover:rotate-90 transition-transform">&times;</button>
                </div>
                <form onSubmit={handleSubmit} className="p-8 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select name="patientId" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none font-bold">
                        <option value="">-- Recette Divers --</option>
                    ...
                    <div className="flex gap-3 pt-4">
                      <button type="button" onClick={onClose} className="flex-1 py-4 text-slate-400 font-black uppercase tracking-widest text-[10px] hover:bg-slate-50 rounded-2xl transition">Annuler</button>
                      <button type="submit" className="flex-[2] py-4 bg-emerald-600 text-white font-black uppercase tracking-widest text-[10px] rounded-2xl shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition">Enregistrer</button>
                    </div>
                </form>
            </div>
        </div>
