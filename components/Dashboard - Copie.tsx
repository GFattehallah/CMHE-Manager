...
import { 
  ...
  Table as TableIcon, TrendingDown, ChevronRight, PieChart as PieIcon, Filter, DollarSign
} from 'lucide-react';
import { Link } from 'react-router-dom';
import { DataService } from '../services/dataService';
import { AppointmentStatus, EXPENSE_CATEGORIES } from '../types';
...
export const Dashboard: React.FC = () => {
  ...
  // KPI: CA Mensuel (Mois en cours de l'année en cours)
    ...
    <div className="p-6 space-y-6 animate-fade-in max-w-[1600px] mx-auto pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">Dr. Hasnaa El Malki • Cabinet Médical CMHE</p>
        </div>
      </div>
      
      {/* 4 Cartes Principales demandées */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div className="flex justify-between items-center mb-8">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Activity size={20} className="text-medical-600"/> Évolution du CA</h3>
                <select 
        ...
        <div className="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl relative overflow-hidden group">
            ...
            <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 mb-6 flex items-center justify-between relative z-10">
                Encaissements Récents <ChevronRight size={18}/>
            </h3>
            <div className="space-y-6 relative z-10">
                {invoices.filter(i => i.status === 'PAID').slice(0, 5).map(inv => (
                    <div key={inv.id} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0">
                        ...
                        <span className="font-black text-lg text-emerald-400 tabular-nums">{formatMAD(inv.amount)}</span>
                    </div>
                ))}
                {invoices.filter(i => i.status === 'PAID').length === 0 && (
                    <p className="text-slate-500 text-xs italic">Aucun encaissement enregistré.</p>
                )}
            </div>
